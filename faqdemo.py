...
async def create_new_faq(insertBeforeID=None, messageStamp=''):
    ...
    title = await chat.ChatInput((("ChatMessageTemplate", f"First, write a 'headline version' of your question as a single sentence, as clearly and simply as you can. (You'll have a chance to explain your question fully in the next step). {messageStamp}"),), insertBeforeID=insertBeforeID).get()
    text = await chat.ChatInput((("ChatMessageTemplate", f"Next, let's nail down exactly what you're unsure about, by applying your question to a real-world situation, to identify what specific outcome you're unsure about (e.g. 'is A going to happen, or B?') {messageStamp}"),), insertBeforeID=insertBeforeID).get()
...
async def update_test(updates, insertBeforeID):
    ...
    await chat.continue_button((("ChatMessageTemplate", f'{faq["answer"]} <span class="chat-new-msg">new</span>'),),
                       insertBeforeID=insertBeforeID)
    hasQuestion = await chat.ChatQuery((("ChatMessageTemplate", 'Have you anything else you are worried about? <span class="chat-new-msg">new</span>'),),
                        'yesno-template', insertBeforeID=insertBeforeID).get()
    if hasQuestion == 'yes':
